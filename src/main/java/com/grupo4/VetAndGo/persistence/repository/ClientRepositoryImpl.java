...
import com.grupo4.VetAndGo.persistence.dao.mapper.ClientMapper;
...
public class ClientRepositoryImpl implements ClientRepository {
    ...
    public List<Client> findAll() {
        return clientJpaDao.findAll().stream()
                .map(ClientMapper::toDomainClient)
                .collect(Collectors.toList());
    }

    @Override
    public Optional<Client> findById(Long id) {
        return clientJpaDao.findById(id).map(ClientMapper::toDomainClient);
    }

    @Override
    public Optional<Client> findByLogin(String login) {
        return clientJpaDao.findByLogin(login).map(ClientMapper::toDomainClient);
    }
}
